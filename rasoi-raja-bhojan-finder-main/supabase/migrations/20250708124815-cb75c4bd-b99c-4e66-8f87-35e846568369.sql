-- Add new delivery stages for better food tracking
-- Note: We cannot add values to existing enum types, so we'll create a new enum
-- Drop the old enum and recreate with all values
...
-- Recreate the deliveries table status column with the new enum
...
-- Set default value
ALTER TABLE public.deliveries 
ALTER COLUMN status SET DEFAULT 'pending_assignment'::public.delivery_status;
